...
#include "SerenityEditor_NewOctreeActor_Dialog.h"
...
Serenity3D_Frame( parent )
{
	...
	material_window->control_id = RC_IRR_WINDOW_MATERIAL;

	setControlContext(NULL);
	createIrrlichtTextureWindow();
	texture_window->control_id = RC_IRR_WINDOW_VIEW2D;

	setControlContext(NULL);
	createIrrlichtAnimationWindow();
	animation_window->control_id = RC_IRR_WINDOW_ANIMATION;
	...
	stage_window->control_id = RC_IRR_WINDOW_NAV3D;
	...
	project.stage_window = stage_window;
	project.animation_window = animation_window;
	project.material_window = material_window;
	project.texture_window = texture_window;
...
void SerenityEditorSerenity3D_Frame::setControlContext(wxIrrlicht* tgt_control)
{
	if(stage_window)
	{
...
void SerenityEditorSerenity3D_Frame::createIrrlichtStageWindow()
{
	...
	project.show_axis_lines = m_toolSettings_showAxisLines_checkBox->GetValue();
	project.show_axis_rings = m_toolSettings_showAxisRings_checkBox->GetValue();
	project.show_camera_pos = m_cameraSettings_showPosition_checkBox->GetValue();
	project.show_camera_rot = m_cameraSettings_showRotation_checkBox->GetValue();
	project.camera_speed = m_cameraSettings_moveSpeed_spinCtrlDouble->GetValue();
	project.hud_color = irr::video::SColor( m_viewHUD_color_colourPicker->GetColour().GetAlpha(),
											m_viewHUD_color_colourPicker->GetColour().GetRed(),
											m_viewHUD_color_colourPicker->GetColour().GetGreen(),
											m_viewHUD_color_colourPicker->GetColour().GetBlue() );
...
void SerenityEditorSerenity3D_Frame::OnMainEditorNotebookPageChanged( wxAuiNotebookEvent& event )
{
	...
	if(current_window)
	{
		...
		project.swapMaterialTexture(current_window);
	}
}
...
void SerenityEditorSerenity3D_Frame::reloadResources()
{
	...
	project.device = stage_window->GetDevice();
...
bool SerenityEditorSerenity3D_Frame::delete_actor(int stage_index, int actor_stage_index)
{
	...
	if(stage_index == stageTab_active_stage_project_index && stage_window != NULL)
	{
		...
		for(int i = 0; i < stage_window->scene_actors.size(); i++)
		{
			...
			{
				stage_window->scene_actors[i].node = NULL;
				stage_window->scene_actors[i].isLight = false;
				stage_window->scene_actors.erase(i, 1);
				break;
			}
		}
	}
...
void SerenityEditorSerenity3D_Frame::On_Stage_DeleteGroup( wxCommandEvent& event )
{
	...
	m_project_stage_treeCtrl->SelectItem(stage_tree_nodes[stage_node_index].tree_item);
}
...
void SerenityEditorSerenity3D_Frame::On_Stage_DeleteActor( wxCommandEvent& event )
{
	...
	wxTreeItemId selected_item = m_project_stage_treeCtrl->GetSelection();

	for(int stage_node_index = 0; stage_node_index < stage_tree_nodes.size(); stage_node_index++)
	{
		for(int i = 0; i < stage_tree_nodes[stage_node_index].actors.size(); i++)
		{
			if(stage_tree_nodes[stage_node_index].actors[i].tree_item == selected_item)
			{
				s_node_index = stage_node_index;
				a_node_index = i;
				break;
			}
		}
	...
	if(a_node_index < 0)
	{
		wxMessageBox(_("Can't delete actor. No actor has been selected"));
		return;
	}

	stage_project_index = stage_tree_nodes[s_node_index].project_index;

	if(stage_project_index < 0 || stage_project_index >= project.stages.size())
		return;

	actor_project_index = stage_tree_nodes[s_node_index].actors[a_node_index].project_index;
	...
	//delete actors and tree items
	//wxMessageBox(_("ACT_I: ") + wxString::Format(_("%d"), actor_project_index));

	delete_actor(stage_project_index, actor_project_index);
	m_project_stage_treeCtrl->Delete(stage_tree_nodes[s_node_index].actors[a_node_index].tree_item);
	stage_tree_nodes[s_node_index].actors[a_node_index].node_type = RC_STAGE_NODE_NONE;
...
void SerenityEditorSerenity3D_Frame::updateProjectFromStageEdit()
{
	...
	for(int i = 0; i < stage_window->selected_actors.size(); i++)
	{
		...
		if(project.stages[stage_project_index].actors[actor_project_index].type == SN_ACTOR_TYPE_TERRAIN)
		{
			project.stages[stage_project_index].actors[actor_project_index].position = stage_window->selected_actors[i].t_pos;
		}
		else
		{
...
void SerenityEditorSerenity3D_Frame::updateRenderMode()
{
	if(!current_window)
		return;
	...
	for(int i = 0; i < project.stages[stageTab_active_stage_project_index].actors.size(); i++)
	{
		...
		for(int mat = 0; mat < project.stages[stageTab_active_stage_project_index].actors[i].node->getMaterialCount(); mat++)
		{
			...
			if(isWireFrame)
			{
				project.stages[stageTab_active_stage_project_index].actors[i].node->getMaterial(mat).BackfaceCulling = false;
				project.stages[stageTab_active_stage_project_index].actors[i].node->getMaterial(mat).FrontfaceCulling = false;
			}
		}
	}
}
...
void SerenityEditorSerenity3D_Frame::On_Stage_RenderMode_Solid( wxCommandEvent& event )
{
	stage_render_mode = 1;
...
void SerenityEditorSerenity3D_Frame::refresh_environmentSettings()
{
	...
	switch(project.stages[stageTab_active_stage_project_index].sky.type)
	{
		...
		case SN_SKY_TYPE_BOX:
		{
			...
			irr::video::ITexture* lf = project.textures[lf_index].texture[SN_IRR_WINDOW_STAGE];
			irr::video::ITexture* rt = project.textures[rt_index].texture[SN_IRR_WINDOW_STAGE];
			irr::video::ITexture* up = project.textures[up_index].texture[SN_IRR_WINDOW_STAGE];
			irr::video::ITexture* dn = project.textures[dn_index].texture[SN_IRR_WINDOW_STAGE];
			irr::video::ITexture* ft = project.textures[ft_index].texture[SN_IRR_WINDOW_STAGE];
			irr::video::ITexture* bk = project.textures[bk_index].texture[SN_IRR_WINDOW_STAGE];

			if(lf == NULL || rt == NULL || up == NULL || dn == NULL || ft == NULL || bk == NULL)
				return;

			project.stages[stageTab_active_stage_project_index].sky.node = stage_window->GetDevice()->getSceneManager()->addSkyBoxSceneNode(up, dn, lf, rt, ft, bk);
		}
		break;
		...
		case SN_SKY_TYPE_DOME:
		{
			...
			irr::video::ITexture* sky_dome = project.textures[img_index].texture[SN_IRR_WINDOW_STAGE];
			...
			project.stages[stageTab_active_stage_project_index].sky.node = stage_window->GetDevice()->getSceneManager()->addSkyDomeSceneNode(sky_dome, hRes, vRes, tx_pct, sphere_pct, radius);
		}
		break;
	}
}
...
void SerenityEditorSerenity3D_Frame::refresh_actor(int actor_project_index)
{
	...
	switch(project.stages[stage_project_index].actors[i].type)
	{
		...
		case SN_ACTOR_TYPE_LIGHT:
		{
			...
			if(project.stages[stage_project_index].actors[i].node != NULL)
			{
				...
				switch(project.stages[stage_project_index].actors[i].light_type)
				{
					case SN_LIGHT_TYPE_POINT:
						node->setLightType(irr::video::ELT_POINT);
						break;
					case SN_LIGHT_TYPE_SPOT:
						node->setLightType(irr::video::ELT_SPOT);
						break;
					case SN_LIGHT_TYPE_DIRECTIONAL:
						node->setLightType(irr::video::ELT_DIRECTIONAL);
						break;
				}
	...
	{
		bool isWireFrame = (stage_render_mode == 0);
		for(int mat = 0; mat < project.stages[stage_project_index].actors[i].node->getMaterialCount(); mat++)
		{
			project.stages[stage_project_index].actors[i].node->getMaterial(mat).Wireframe = isWireFrame;

			if(isWireFrame)
			{
				project.stages[stage_project_index].actors[i].node->getMaterial(mat).BackfaceCulling = false;
				project.stages[stage_project_index].actors[i].node->getMaterial(mat).FrontfaceCulling = false;
			}
		}
		...
		if(project.stages[stage_project_index].actors[i].icon_node)
		{
			...
			project.stages[stage_project_index].actors[i].icon_node->setPosition(irr::core::vector3df(0,0,0));
		...
		scene_obj.node = node;
		scene_obj.icon_node = project.stages[stage_project_index].actors[i].icon_node;
		scene_obj.rotation = &project.stages[stage_project_index].actors[i].rotation;
		scene_obj.isLight = (project.stages[stage_project_index].actors[i].type == SN_ACTOR_TYPE_LIGHT);
		scene_obj.isTerrain = (project.stages[stage_project_index].actors[i].type == SN_ACTOR_TYPE_TERRAIN);
		scene_obj.t_pos = project.stages[stage_project_index].actors[i].position;

		if(scene_obj.icon_node != NULL && use_icon_bbox)
		{
		...
		for(int scene_actors_n=0; scene_actors_n < stage_window->scene_actors.size(); scene_actors_n++)
		{
			if(stage_window->scene_actors[scene_actors_n].actor_index == i)
			{
	...
	for(int sn = 0; sn < stage_window->scene_actors.size(); sn++)
	{
		if(stage_window->scene_actors[sn].actor_index == i)
		{
			stage_window->scene_actors[sn].node = project.stages[stage_project_index].actors[i].node;
			stage_window->scene_actors[sn].icon_node = project.stages[stage_project_index].actors[i].icon_node;
			stage_window->scene_actors[sn].t_pos = project.stages[stage_project_index].actors[i].position;
			stage_window->scene_actors[sn].t_start = project.stages[stage_project_index].actors[i].position;
		}
	}

	for(int sn = 0; sn < stage_window->selected_actors.size(); sn++)
	{
		if(stage_window->selected_actors[sn].actor_index == i)
		{
			stage_window->selected_actors[sn].node = project.stages[stage_project_index].actors[i].node;
			stage_window->selected_actors[sn].icon_node = project.stages[stage_project_index].actors[i].icon_node;
			stage_window->selected_actors[sn].t_pos = project.stages[stage_project_index].actors[i].position;
			stage_window->selected_actors[sn].t_start = project.stages[stage_project_index].actors[i].position;
		}
	}
}
...
void SerenityEditorSerenity3D_Frame::OnStagePropertyGridChanged( wxPropertyGridEvent& event )
{
	...
	else if(property_name.compare(_("hmap_image_id"))==0)
	{
		...
		img_id = page->GetPropertyByName(_("hmap_image_id"))->GetValueAsString();
	...
	else if(property_name.compare(_("outer_cone"))==0)
	{
		...
		if(stage_index == stageTab_active_stage_project_index)
		{
			project.stages[stage_index].actors[actor_index].icon_node->remove();
			project.stages[stage_index].actors[actor_index].icon_node = NULL;
			project.stages[stage_index].actors[actor_index].node->remove();
			project.stages[stage_index].actors[actor_index].node = NULL;
			refresh_actor(actor_index);
		}
	...
	else if(property_name.compare(_("material_id"))==0)
	{
		...
		//std::cout << "MAT_ID = " << mat_id.ToStdString() << std::endl;
		...
		if(stage_index == stageTab_active_stage_project_index)
		{
			if(project.stages[stage_index].actors[actor_index].node)
			{
				...
				if(project.stages[stage_index].actors[actor_index].override_material_index >= 0 && project.stages[stage_index].actors[actor_index].override_material_index < project.materials.size())
				{
		...
		case RC_STAGE_NODE_ACTOR:
		{
			...
			if(stage_window != NULL && stage_project_index == stageTab_active_stage_project_index)
			{
				...
				if(!is_selected)
				{
					...
					scene_obj.icon_node = project.stages[stage_project_index].actors[actor_stage_index].icon_node;
					scene_obj.rotation = &project.stages[stage_project_index].actors[actor_stage_index].rotation;
					scene_obj.isLight = (project.stages[stage_project_index].actors[actor_stage_index].type == SN_ACTOR_TYPE_LIGHT);
					scene_obj.isTerrain = (project.stages[stage_project_index].actors[actor_stage_index].type == SN_ACTOR_TYPE_TERRAIN);
					scene_obj.t_pos = project.stages[stage_project_index].actors[actor_stage_index].position;
...
{
	if(stage_window)
		stage_window->show_axis_lines = event.IsChecked();
...
{
	if(stage_window)
		stage_window->show_camera_pos = event.IsChecked();
...
{
	if(stage_window)
		stage_window->show_camera_rot = event.IsChecked();
...
{
	if(stage_window)
		stage_window->cam_move_speed = event.GetValue();
...
{
	if(stage_window)
		stage_window->hud_color = irr::video::SColor( event.GetColour().GetAlpha(),
														event.GetColour().GetRed(),
														event.GetColour().GetGreen(),
...
void SerenityEditorSerenity3D_Frame::OnS3DSelectClicked( wxCommandEvent& event )
{
	...
	updateToolSelection();
	stage_window->stage_edit_tool = RC_EDIT_TOOL_SELECT;
}
...
void SerenityEditorSerenity3D_Frame::OnS3DBoxClicked( wxCommandEvent& event )
{
	...
	updateToolSelection();
	stage_window->stage_edit_tool = RC_EDIT_TOOL_BOXSELECT;
}
...
void SerenityEditorSerenity3D_Frame::OnS3DRotateClicked( wxCommandEvent& event )
{
	...
	updateToolSelection();
	stage_window->stage_edit_tool = RC_EDIT_TOOL_ROTATE;
}
...
void SerenityEditorSerenity3D_Frame::OnS3DCubeClicked( wxCommandEvent& event )
{
	...
	if(stageTab_active_stage_project_index < 0 || stageTab_active_stage_project_index >= project.stages.size())
	{
		wxMessageBox(_("Must have an active stage to create an actor in"));
		return;
	}
	...
	double cube_size = dialog->cube_size;
...
void SerenityEditorSerenity3D_Frame::OnS3DDumpClicked( wxCommandEvent& event )
{
	...
	SerenityEditor_NewOctreeActor_Dialog* dialog = new SerenityEditor_NewOctreeActor_Dialog(this);
...
void SerenityEditorSerenity3D_Frame::OnS3DWizClicked( wxCommandEvent& event )
{
	...
	SerenityEditor_NewAnimatedActor_Dialog* dialog = new SerenityEditor_NewAnimatedActor_Dialog(this);
...
void SerenityEditorSerenity3D_Frame::OnS3DBillboardClicked( wxCommandEvent& event )
{
	...
	SerenityEditor_NewBillboardActor_Dialog* dialog = new SerenityEditor_NewBillboardActor_Dialog(this);

	for(int i = 0; i < project.materials.size(); i++)
	{
		dialog->materials.push_back(project.materials[i].id_name);
	}
	...
	wxString material_id_name = dialog->material_id_name;

	for(int i = 0; i < project.materials.size(); i++)
	{
		if(project.materials[i].id_name.compare(material_id_name.ToStdString())==0)
		{
			material_index = i;
			break;
		}
	...
	int actor_project_index = project.stages[stageTab_active_stage_project_index].addActor(actor_id.ToStdString(), SN_ACTOR_TYPE_BILLBOARD);

	project.stages[stageTab_active_stage_project_index].actors[actor_project_index].physics.isSolid = false;
	project.stages[stageTab_active_stage_project_index].actors[actor_project_index].physics.mass = 0;
	project.stages[stageTab_active_stage_project_index].actors[actor_project_index].physics.shape = SN_PHYSICS_SHAPE_BOX;

	project.stages[stageTab_active_stage_project_index].actors[actor_project_index].override_material_index = material_index;
...
void SerenityEditorSerenity3D_Frame::OnS3DLightClicked( wxCommandEvent& event )
{
	...
	SerenityEditor_NewLightActor_Dialog* dialog = new SerenityEditor_NewLightActor_Dialog(this);

	for(int i = 0; i < SN_LIGHT_TYPE_COUNT; i++)
	{
		dialog->light_type_list.push_back(project.getLightTypeString(i));
	}
	...
	int light_type = project.getLightType(dialog->light_type);
	...
	int actor_project_index = project.stages[stageTab_active_stage_project_index].addActor(actor_id.ToStdString(), SN_ACTOR_TYPE_LIGHT);

	project.stages[stageTab_active_stage_project_index].actors[actor_project_index].physics.isSolid = false;
	project.stages[stageTab_active_stage_project_index].actors[actor_project_index].physics.mass = 0;
	project.stages[stageTab_active_stage_project_index].actors[actor_project_index].physics.shape = SN_PHYSICS_SHAPE_SPHERE;

	project.stages[stageTab_active_stage_project_index].actors[actor_project_index].light_type = light_type;
...
bool SerenityEditorSerenity3D_Frame::load_project(wxFileName pfile)
{
	...
	project = serenity_project(pfile.GetAbsolutePath().ToStdString(), "", stage_window->GetDevice(), stage_window, animation_window, material_window, texture_window);
...
void SerenityEditorSerenity3D_Frame::On_Mesh_RemoveMaterial( wxCommandEvent& event )
{
	...
	if(project.meshes[n].material_index.size() == 0)
		return;
...
void SerenityEditorSerenity3D_Frame::On_Material_SetTextureLevel_ButtonClicked( wxCommandEvent& event )
{
	...
	for(int i = 0; i < project.textures.size(); i++)
	{
		if(project.textures[i].id_name.compare(selected_texture.ToStdString())==0)
		{
			...
			if(project.textures[i].texture[SN_IRR_WINDOW_MATERIAL])
				project.materials[n].material.setTexture(t_level, project.textures[i].texture[SN_IRR_WINDOW_MATERIAL]);
...
void SerenityEditorSerenity3D_Frame::On_Texture_RemoveTexture_ButtonClicked( wxCommandEvent& event )
{
	...
	texture_window->view2D_texture = NULL;
	...
	if(new_selection >= 0)
	{
		...
		for(int i = 0; i < project.textures.size(); i++)
		{
			if(project.textures[i].id_name.compare(t_select.ToStdString())==0)
			{
				...
				if(texture_window)
					texture_window->view2D_texture = project.textures[i].texture[SN_IRR_WINDOW_TEXTURE];
...
void SerenityEditorSerenity3D_Frame::On_Texture_TextureList_Select( wxCommandEvent& event )
{
	...
	if(texture_window)
	{
		//texture_window->view2D_texture = project.textures[n].texture;
		if(texture_window->view2D_texture)
		{
			texture_window->GetDevice()->getVideoDriver()->removeTexture(texture_window->view2D_texture);
			texture_window->view2D_texture = NULL;
		}

		wxFileName fname = project.project_path;
		fname.AppendDir(_("textures"));
		fname.SetFullName(wxString(project.textures[n].file));
		//std::cout << "Load Texture: " << fname.GetAbsolutePath() << std::endl;
		texture_window->view2D_texture = texture_window->GetDevice()->getVideoDriver()->getTexture(fname.GetAbsolutePath().ToStdString().c_str());
	}
}
